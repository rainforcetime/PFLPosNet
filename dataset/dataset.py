...
class ReactionDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        listener_personal_path = self.choose_personal_path(index)
        # listener_personal_path = os.path.join('/'.join(self.listener_path[index].split('/')[:3]), str(personal_id))
        ...
        # # print(speaker_3dmm_clip.shape) # (750,58)
        # speaker_3dmm_clip[0:125,:] = speaker_3dmm_clip[-125:,:]
        # speaker_audio_clip[0:125,:] = speaker_audio_clip[-125:,:]
        # speaker_emotion_clip[0:125,:] = speaker_emotion_clip[-125:,:]
        # speaker_video_clip[0:125,:] = speaker_video_clip[-125:,:]
        return (
            speaker_audio_clip,
            speaker_video_clip,
            speaker_emotion_clip,
    ...
    def choose_personal_path(self, index):
        choose_id = 1
        listener_personal_path = os.path.join('/'.join(self.listener_path[index].split('/')[:3]), str(choose_id))
        full_path = os.path.join(self._3dmm_path, listener_personal_path + '.npy')
        while not os.path.exists(full_path):
            choose_id += 1
            listener_personal_path = os.path.join('/'.join(self.listener_path[index].split('/')[:3]), str(choose_id))
            full_path = os.path.join(self._3dmm_path, listener_personal_path + '.npy')
        return listener_personal_path
