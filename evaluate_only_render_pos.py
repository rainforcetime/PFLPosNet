...
def evaluate(cfg, device, model, test_loader, render, split):
    ...
    for batch_idx, (
        ...
        if batch_idx not in [10, 110, 120, 460, 490, 580]:
            continue
        ...
        if (batch_idx % cfg.test_period) == 0:
            with torch.no_grad():
                ...
                print("listener_3dmm_pred.shape: ", listener_3dmm_pred.shape)

                # print("listener_3dmm_pred[0, 0].shape: ", listener_3dmm_pred[0, 0].shape)
                # listener_3dmm_pred[0, 0][:, 52:] = listener_3dmm_pred[0, 0][10, 52:]

                # TODO: Rendering!
                render.rendering_for_fid_simple(
                    out_dir,
