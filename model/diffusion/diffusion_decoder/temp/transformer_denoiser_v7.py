...
class TransformerDenoiser_v7(nn.Module):
    def __init__(self,
        ...
        self.to_3dmm_expr_embed = nn.Linear(expr_dim, self.latent_dim) if expr_dim != self.latent_dim else nn.Identity()
        self.to_3dmm_pos_embed = nn.Linear(pos_dim, self.latent_dim) if pos_dim != self.latent_dim else nn.Identity()

        self.to_3dmm_expr = nn.Linear(self.latent_dim, self.expr_dim) if self.latent_dim != self.expr_dim else nn.Identity()
        self.to_3dmm_pos = nn.Linear(self.latent_dim, self.pos_dim) if self.latent_dim != self.expr_dim else nn.Identity()
        self.pos_fusion = nn.Linear(self.pos_dim * 2, self.pos_dim) if self.pos_dim != self.latent_dim else nn.Identity()
        ...
        if self.encode_3dmm:  # assume dimension of encoded 3dmm equals latent_dim
            ...
            self.speaker_3dmm_proj = nn.Linear(nfeats, self.latent_dim)
    ...
    def get_model_kwargs(
        ...
        if past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0:
            ...
            past_listener_3dmm_pos = torch.zeros(size=(bs, sample.shape[1], self.pos_dim)).to(sample.device)
        else:
            past_listener_3dmm_pos = past_listener_3dmmenc[:, :, - self.pos_dim:]
    ...
    def _forward(
        ...
        pos = self.to_3dmm_pos(sample_pos)
        pos = torch.cat((pos, past_listener_3dmm_pos), dim=-1)
        pos = self.pos_fusion(pos)

        expr = self.to_3dmm_expr(sample_expr)

        sample = torch.cat((expr, pos), dim=-1)
