...
class TransformerDenoiser_v6(nn.Module):
    def __init__(self,
        ...
        self.to_3dmm = nn.Linear(self.latent_dim, nfeats) if self.latent_dim != nfeats else nn.Identity()
    ...
    def _forward(
        ...
        sample = self.to_3dmm(sample)
        pos = torch.cat((sample[:,:,-self.pos_dim:], past_listener_3dmmenc_pos), dim=-1)
        pos = self.pos_fusion(pos)

        sample[:,:,-self.pos_dim:] = pos
