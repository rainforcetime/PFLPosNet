...
class TransformerDenoiser_v3(nn.Module):
    def __init__(self,
        ...
        self.to_3dmm_expr_embed = nn.Linear(expr_dim, self.latent_dim) if nfeats != self.latent_dim else nn.Identity()
        self.to_3dmm_pos_embed = nn.Linear(pos_dim, self.latent_dim) if nfeats != self.latent_dim else nn.Identity()

        self.to_3dmm_feat = nn.Linear(self.latent_dim * 2, nfeats) if self.latent_dim != nfeats else nn.Identity()
        ...
        if self.encode_3dmm:  # assume dimension of encoded 3dmm equals latent_dim
            self.speaker_expr_3dmm_proj = nn.Identity()
            self.speaker_pos_3dmm_proj = nn.Identity()
        else:
            assert s_3dmm_dim != self.latent_dim, "wrong dimension of raw 3dmm features."
            self.speaker_expr_3dmm_proj = nn.Linear(expr_dim, self.latent_dim)
            self.speaker_pos_3dmm_proj = nn.Linear(pos_dim, self.latent_dim)
        ...
        self.listener_expr_3dmm_proj = nn.Linear(expr_dim, self.latent_dim) \
            if nfeats != self.latent_dim else nn.Identity()

        self.listener_pos_3dmm_proj = nn.Linear(pos_dim, self.latent_dim) \
            if nfeats != self.latent_dim else nn.Identity()
        ...
        if self.arch == "trans_enc":  # Transformer Encoder
            ...
            self.decoder_pos = TransformerDecoder(
                decoder_layer,
                num_layers,
                nn.LayerNorm(self.latent_dim),
                return_intermediate=return_intermediate_dec,
    ...
    def get_model_kwargs(
        ...
        if speaker_3dmm_encodings is None or self.s_3dmm_enc_drop_prob >= 1.0:
            speaker_3dmm_expr_encodings = torch.zeros(size=(bs, 0, self.latent_dim)).to(sample.device)
            speaker_3dmm_pos_encodings = torch.zeros(size=(bs, 0, self.latent_dim)).to(sample.device)
        else:
            # print("speaker_3dmm_encodings shape", speaker_3dmm_encodings.shape)
            speaker_3dmm_expr_encodings = self.speaker_expr_3dmm_proj(speaker_3dmm_encodings[:, :, :self.expr_dim])
            speaker_3dmm_pos_encodings = self.speaker_pos_3dmm_proj(speaker_3dmm_encodings[:, :, self.expr_dim:])
            speaker_3dmm_expr_encodings = self.mask_cond(speaker_3dmm_expr_encodings, mode, self.s_3dmm_enc_drop_prob)
            speaker_3dmm_pos_encodings = self.mask_cond(speaker_3dmm_pos_encodings, mode, self.s_3dmm_enc_drop_prob)
        ...
        if past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0:
            past_listener_3dmmenc_expr = torch.zeros(size=(bs, 0, self.latent_dim)).to(sample.device)
            past_listener_3dmmenc_pos = torch.zeros(size=(bs, 0, self.latent_dim)).to(sample.device)
        else:
            # TODO: use the shared MLP with speaker 3dmm or use its own unique MLP.
            # print("past_listener_3dmmenc shape", past_listener_3dmmenc.shape)
            past_listener_3dmmenc_expr = self.listener_expr_3dmm_proj(past_listener_3dmmenc[:, :, :self.expr_dim])  # we use its unique MLP.
            past_listener_3dmmenc_pos = self.listener_pos_3dmm_proj(past_listener_3dmmenc[:, :, self.expr_dim:])  # we use its unique MLP.
            ...
            past_listener_3dmmenc_expr = self.mask_cond(past_listener_3dmmenc_expr, mode, self.past_l_3dmm_drop_prob)
            past_listener_3dmmenc_pos = self.mask_cond(past_listener_3dmmenc_pos, mode, self.past_l_3dmm_drop_prob)
        ...
        return (listener_latent_embed,
                ...
                speaker_3dmm_expr_encodings,
                speaker_3dmm_pos_encodings,
                speaker_emotion_encodings,
                past_listener_3dmmenc_expr,
                past_listener_3dmmenc_pos)
    ...
    def _forward(
        ...
        # print("past_listener_3dmmenc", past_listener_3dmmenc.shape)
        # 5 / 0
        ...
        sample_expr = sample[:, :, :self.expr_dim]
        sample_pos = sample[:, :, self.expr_dim:]

        sample_expr = self.to_3dmm_expr_embed(sample_expr)
        sample_pos = self.to_3dmm_pos_embed(sample_pos)
        ...
        if self.use_attn_mask:
            ...
            # todo we assume all condition's sep_len is 50.
            # assert seq_len == speaker_audio_encodings.shape[1], \
            #     "dimension inconsistency between noised input and speaker_audio_encodings"
            # assert seq_len == speaker_3dmm_encodings.shape[1], \
            #     "dimension inconsistency between noised input and speaker_3dmm_encodings"
            # assert seq_len == past_listener_3dmmenc.shape[1], \
            #     "dimension inconsistency between noised input and past_listener_3dmmenc"
        ...
        # 2. we concat conditions along token dim.
            ...
            if self.arch == "trans_enc":
                ...
                if self.position_embedding != 'none':
                    ...
                    speaker_3dmm_expr_encodings = self.mem_pos(speaker_3dmm_expr_encodings)
                    speaker_3dmm_pos_encodings = self.mem_pos(speaker_3dmm_pos_encodings)
                    if past_listener_3dmmenc_expr.shape[1] > 0:
                        past_listener_3dmmenc_expr = self.mem_pos(past_listener_3dmmenc_expr)
                        past_listener_3dmmenc_pos = self.mem_pos(past_listener_3dmmenc_pos)

                emb_latent_expr = torch.cat((
                    time_embed,
                    speaker_audio_encodings,
                    speaker_3dmm_expr_encodings,
                    past_listener_3dmmenc_expr,
                ...
                sample_expr = self.query_pos(sample_expr)
                sample_pos = self.query_pos(sample_pos)
                ...
                sample_expr = self.decoder_expr(tgt=sample_expr, memory=emb_latent_expr,
                                      tgt_mask=tgt_mask, memory_mask=memory_mask).squeeze(0)

                sample_pos = self.decoder_pos(tgt=sample_pos, memory=emb_latent_pos,
                                      tgt_mask=tgt_mask, memory_mask=memory_mask).squeeze(0)
        ...
        # print("sample shape", sample.shape) # [20,50,512]
        # TODO 或许可以加一个融合层用于融合表情和位姿信息 当前的融合方式是直接拼接
        sample = self.to_3dmm_feat(torch.cat((sample_expr, sample_pos), dim=-1))
