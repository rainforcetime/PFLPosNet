...
from model.diffusion.diffusion_decoder.temp.pose import PoseGenerator
...
class TransformerDenoiser_v14(nn.Module):
    def __init__(self,
        ...
        # self.to_3dmm = nn.Linear(self.latent_dim + self.pos_dim, nfeats) if self.latent_dim != nfeats else nn.Identity()
        self.pose_generator = PoseGenerator(window_size, slide_size, self.latent_dim, hidden_size=128, pose_dim=self.pos_dim)
    ...
    def get_model_kwargs(
        ...
        if past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0:
            ...
            past_listener_3dmmenc_pos = torch.zeros(size=(bs, sample.shape[1], self.pos_dim)).to(sample.device)
        else:
            past_listener_3dmmenc_pos = past_listener_3dmmenc[:, :, - self.pos_dim:]
    ...
    def _forward(
        ...
        expr = self.to_3dmm_expr(sample)
        pose = self.pose_generator(past_listener_3dmmenc_pos, sample)
        overlap_size = self.window_size - self.slide_size
        pose = torch.cat((past_listener_3dmmenc_pos[:, -overlap_size:, :], pose), dim=1)

        sample = torch.cat((expr, pose), dim=-1)
