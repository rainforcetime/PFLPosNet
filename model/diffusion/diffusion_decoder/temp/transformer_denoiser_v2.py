...
from model.diffusion.diffusion_decoder.temp.expr_rot_trans_decoder import ExprRotTransDecoder
...
class TransformerDenoiser_v2(nn.Module):
    def __init__(self,
        ...
        self.to_3dmm_feat = ExprRotTransDecoder(self.latent_dim, nfeats, expr_dim, rot_trans_dim)

        # self.to_3dmm_expr_feat = nn.Linear(self.latent_dim, expr_dim) if self.latent_dim != nfeats else nn.Identity()
        # self.to_3dmm_rot_feat = nn.Linear(self.latent_dim + expr_dim, rot_trans_dim // 2) if self.latent_dim != nfeats else nn.Identity()
        # self.to_3dmm_trans_feat = nn.Linear(self.latent_dim + expr_dim, rot_trans_dim // 2) if self.latent_dim != nfeats else nn.Identity()
    ...
    def _forward(
        ...
        # print("sample shape", sample.shape) # [20,50,58]
        # expr_sample = self.to_3dmm_expr_feat(sample)
        # concat_sample = torch.cat((sample, expr_sample), dim=-1)
        #
        # rot_trans_sample = self.to_3dmm_rot_trans_feat(concat_sample)
        #
        # sample = torch.cat((expr_sample, rot_trans_sample), dim=-1)
