...
class TransformerDenoiser_v9(nn.Module):
    def __init__(self,
        ...
        self.pos_dim = nfeats - expr_dim
        self.rot_dim = self.pos_dim // 2
        self.trans_dim = self.pos_dim // 2
        ...
        self.to_3dmm_expr_rot = nn.Linear(self.latent_dim, self.expr_dim + self.rot_dim) if self.latent_dim != (self.expr_dim + self.rot_dim) else nn.Identity()
        self.to_3dmm_trans = nn.Linear(self.latent_dim, self.trans_dim) if self.latent_dim != self.trans_dim else nn.Identity()
        self.trans_fusion = nn.Linear(self.trans_dim * 2, self.trans_dim) if self.trans_dim != self.latent_dim else nn.Identity()
    ...
    def get_model_kwargs(
        ...
        listener_reference_depth = model_kwargs.get('listener_reference_depth')
        ...
        if past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0:
            # print("past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0")
            past_listener_3dmmenc = torch.zeros(size=(bs, 0, self.latent_dim)).to(sample.device)
            past_listener_3dmmenc_trans = torch.zeros(size=(bs, sample.shape[1], self.trans_dim)).to(sample.device)

        else:
            past_listener_3dmmenc_trans = past_listener_3dmmenc[:, :, - self.trans_dim:]
    ...
    def _forward(
        ...
        # 2. we concat conditions along token dim.
            ...
            if self.arch == "trans_enc":
                ...
                # print("time_embed shape", time_embed.shape) # [20, 1, 512]
                # print("speaker_audio_encodings shape", speaker_audio_encodings.shape) # [20, 50, 512]
                # print("speaker_3dmm_encodings shape", speaker_3dmm_encodings.shape) # [20, 50, 512]
                # print("past_listener_3dmmenc shape", past_listener_3dmmenc.shape) # [20, 50, 512]
                ...
                # print("emb_latent shape", emb_latent.shape) # [20, 151, 512]
        ...
        trans = self.to_3dmm_trans(sample)
        trans = torch.cat((trans, past_listener_3dmmenc_trans), dim=-1)
        trans = self.trans_fusion(trans)

        expr = self.to_3dmm_expr_rot(sample)

        sample = torch.cat((expr, trans), dim=-1)
