...
class TransformerDenoiser_v8(nn.Module):
    def __init__(self,
    ...
    def get_model_kwargs(
        ...
        if past_listener_3dmmenc is None or self.past_l_3dmm_drop_prob >= 1.0:
            ...
            past_listener_3dmmenc_pos = torch.zeros(size=(bs, T, self.pos_dim)).to(sample.device)
        else:
            past_listener_3dmmenc_pos = past_listener_3dmmenc[:, -1, - self.pos_dim:].unsqueeze(1).expand(-1, T, -1)
            # print("past_listener_3dmmenc_pos shape", past_listener_3dmmenc_pos.shape)
    ...
    def _forward(
        ...
        pos = self.to_3dmm_pos(sample)
        pos = torch.cat((pos, past_listener_3dmmenc_pos), dim=-1)
