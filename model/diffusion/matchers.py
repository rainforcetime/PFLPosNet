...
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v10 import TransformerDenoiser_v10
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v11 import TransformerDenoiser_v11
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v12 import TransformerDenoiser_v12
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v13 import TransformerDenoiser_v13
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v14 import TransformerDenoiser_v14
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v3 import TransformerDenoiser_v3
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v4 import TransformerDenoiser_v4
from model.diffusion.diffusion_decoder.transformer_denoiser_v5 import TransformerDenoiser_v5
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v6 import TransformerDenoiser_v6
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v7 import TransformerDenoiser_v7
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v8 import TransformerDenoiser_v8
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v9 import TransformerDenoiser_v9
...
from model.diffusion.diffusion_decoder.temp.transformer_denoiser_v2 import TransformerDenoiser_v2
...
from utils.util import checkpoint_resume, checkpoint_load
...
class PriorLatentMatcher(BaseLatentModel):
    ...
    def _forward(
        ...
        if self.mode in ["train", "val"]:  # sample random sub-window for each batch element
            ...
            output_pred = self.prior_diffusion.denoise(self.model, x_start, t, model_kwargs=model_kwargs) # dict

            # print("_________djafjdfjdhkdfkhasdkf______")
            # print(output_pred.keys()) # dict_keys(['encoded_prediction', 'encoded_target'])
            # # print(output_pred['encoded_prediction'].shape) #torch.Size([2, 10, 1, 512])
            # print(output_pred['encoded_target'].shape) # torch.Size([2, 10, 1, 512])
...
def non_linear_interpolation(t, start, end, delay_size):
    """
    使用二次函数进行非线性插值。

    :param t: 当前时间步索引 (0 <= t < delay_size)
    :param start: 起始位置 (past_pos)
    :param end: 结束位置 (this_window[:, :, delay_size, -6:])
    :param delay_size: 总的时间步数
    :return: 插值结果
    """
    # 定义一个简单的二次函数用于插值
    # 这里假设非线性插值遵循 y = a*t^2 + b*t + c 的形式
    # 我们需要确定系数a, b, c使得t=0时y=start, t=delay_size时y=end，并且保持连续性

    # 计算二次函数的参数
    a = (end - start) / (delay_size ** 2)
    b = -2 * a * delay_size
    c = start
...
def delay_cat_nonlinear(this_window: torch.Tensor, delay_size: int, past_pos: torch.Tensor) -> torch.Tensor:
    """
    对 this_window 的最后6个元素执行非线性插值，基于 past_pos 和指定的延迟大小。

    :param this_window: [1, 10, 25, 58] 形状的张量。
    :param delay_size: 延迟大小，int 类型。
    :param past_pos: [1, 10, 6] 形状的张量。
    :return: 更新后的 this_window 张量。
    """
    if delay_size == 0:
        return this_window

    B, K, T, D = this_window.shape

    # 检查输入形状是否匹配预期
    if past_pos.shape != (B, K, 6):
        raise ValueError(f"past_pos 的形状应为 [{B}, {K}, 6]，但得到了 {past_pos.shape}")

    # 获取结束位置
    end_pos = this_window[:, :, delay_size, -6:]

    # 创建一个索引序列用于插值操作
    steps = torch.arange(delay_size, device=this_window.device).view(1, 1, delay_size, 1)

    # 广播并计算新的值
    updated_values = non_linear_interpolation(steps.float(), past_pos.unsqueeze(2), end_pos.unsqueeze(2), delay_size)

    # 更新 this_window 的相应部分
    this_window[:, :, :delay_size, -6:] = updated_values

    return this_window


def cubic_spline_interpolation(t, start, end, delay_size):
    """
    使用自然三次样条插值进行平滑插值。
    ...
    """
    # 定义自然三次样条插值的参数
    # 对于两个点 (0, start) 和 (delay_size, end)，我们可以求解出三次多项式的系数

    # 系数 a, b, c, d 分别对应 y = a*t^3 + b*t^2 + c*t + d
    a = (end - start) / (delay_size ** 3)
    b = -3 * a * delay_size
    c = 3 * a * delay_size ** 2
    d = start
...
def delay_cat_cubic_spline(this_window: torch.Tensor, delay_size: int, past_pos: torch.Tensor) -> torch.Tensor:
    """
    对 this_window 的最后6个元素执行基于三次样条插值的平滑插值，基于 past_pos 和指定的延迟大小。
    ...
    # 创建一个索引序列用于插值操作
    steps = torch.arange(delay_size, device=this_window.device).view(1, 1, delay_size, 1).float()

    # 广播并计算新的值
    updated_values = cubic_spline_interpolation(steps, past_pos.unsqueeze(2), end_pos.unsqueeze(2), delay_size)
...
def delay_cat(this_window: torch.Tensor, delay_size: int, past_pos: torch.Tensor) -> torch.Tensor:
    """
    对 this_window 的最后6个元素执行线性插值，基于 past_pos 和指定的延迟大小。
    ...
    # 计算延迟间隙
    delay_gap = this_window[:, :, delay_size, -6:] - past_pos

    # 如果 delay_size 是 0，则 per_step 会变成未定义，所以这里我们假设 delay_size > 0
    per_step = delay_gap / delay_size

    # 创建一个索引序列用于乘法操作
    steps = torch.arange(1, delay_size + 1, device=this_window.device).view(1, 1, delay_size, 1)

    # 广播并计算新的值
    updated_values = past_pos.unsqueeze(2) + per_step.unsqueeze(2) * steps
...
class DecoderLatentMatcher(BaseLatentModel):
    ...
    def __init__(self, conf, device):
        ...
        self.emotion_decode = cfg.get("emotion_decode", False)

        self.warmup_ended = False  # 默认预热未结束

        self.train_sliding = cfg.get("train_sliding", False)

        self.overlap_concat = cfg.get("overlap_concat", True)

        # self.use_detect_depth = cfg.get("use_detect_depth", False)

        self.use_interpolation_inference = cfg.get("use_interpolation_inference", False)

        self.is_concat_inference = cfg.get("is_concat_inference", True)
        ...
        self.use_past_sliding = cfg.get("use_past_sliding", False)
        self.latent_dim = cfg.get("latent_dim", 512)

        self.init_params = {
            "emotion_decode": self.emotion_decode,
            "use_past_sliding": self.use_past_sliding,
            "train_sliding": self.train_sliding,
        ...
        if conf.diffusion_decoder.type == "TransformerDenoiser":
            self.model = TransformerDenoiser(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v2":
            self.model = TransformerDenoiser_v2(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v3":
            self.model = TransformerDenoiser_v3(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v4":
            self.model = TransformerDenoiser_v4(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v5":
            self.model = TransformerDenoiser_v5(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v6":
            self.model = TransformerDenoiser_v6(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v7":
            self.model = TransformerDenoiser_v7(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v8":
            self.model = TransformerDenoiser_v8(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v9":
            self.model = TransformerDenoiser_v9(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v10":
            self.model = TransformerDenoiser_v10(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v11":
            self.model = TransformerDenoiser_v11(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v12":
            self.model = TransformerDenoiser_v12(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v13":
            self.model = TransformerDenoiser_v13(**self.init_params)
        elif conf.diffusion_decoder.type == "TransformerDenoiser_v14":
            self.model = TransformerDenoiser_v14(**self.init_params)

        # if self.use_detect_depth:
        #     depth_cfg = conf.depth_detector.args
    ...
    def _forward(
            ...
            listener_reference = None,  # listener's reference pic.
    ):
        ...
        if self.mode in ["train", "val"]:
            ...
            if self.train_sliding:
                past_listener_3dmm = torch.cat((
                    torch.zeros(size=(l_batch, (self.window_sliding - self.reaction_delay), _3dmm_dim)).to(
                        listener_3dmm_input.device),
                    listener_3dmm_input[:, :(self.token_len + self.reaction_delay - self.window_size - self.window_sliding)],
            ...
            if self.use_past_sliding:
                # we use generated past listener 3dmm as condition
                if window_start > (seq_len - (self.window_size + self.window_sliding)):
                    window_start = (seq_len - (self.window_size + self.window_sliding))
            ...
            output_whole = self.decoder_diffusion.denoise(self.model, x_start_selected, timesteps,
                                                          model_kwargs=model_kwargs)
            output_whole = {
                "prediction_3dmm": output_whole["prediction_3dmm"],
                "target_3dmm": output_whole["target_3dmm"],
                "past_listener_3dmmenc": past_listener_3dmmenc,
            }

            if self.use_generated_pl and self.warmup_ended:  # we do feed-forward again, but based on conditions after a period (window_size).
                ...
                # shape: (batch_size * k, window_size, ...)
                l_emotion_selected = listener_emotion_shifted[:, window_start:window_end]
                # print(l_emotion_selected.shape)
                l_3dmm_selected = listener_3dmm_shifted[:, window_start:window_end]
                ...
                with torch.no_grad():
                    ...
                    # print(output_whole['prediction_3dmm'].shape)
                    # print(output['prediction_3dmm'].shape)
                    prediction_3dmm = torch.cat((
                        output_whole['prediction_3dmm'], output['prediction_3dmm']
                    ), dim=-2)  # (B, k, window_size + (window_size - window_sliding), d)
            ...
            # print(self.warmup_ended)
            if self.use_past_sliding and self.warmup_ended:  # we do feed-forward again, but based on conditions after a period (window_size).
                window_start = window_start + self.window_sliding
                window_end = window_start + self.window_size
                ...
                with torch.no_grad():
                    ...
                    # print(output['prediction_3dmm'].shape)
                    prediction_3dmm = torch.cat((
                        output_whole['prediction_3dmm'], output['prediction_3dmm'][:, :, -self.window_sliding:, :]
                    ), dim=-2)  # (B, k, window_size + (window_size - window_sliding), d)
                    target_3dmm = torch.cat((
                        output_whole['target_3dmm'], output['target_3dmm'][:, :, -self.window_sliding:, :]
                    ), dim=-2)

                    prediction_3dmm_overlap_pre = output_whole['prediction_3dmm'][:, :, -self.window_sliding:, :]
                    prediction_3dmm_overlap_this = output['prediction_3dmm'][:, :, :-self.window_sliding, :]

                    output_whole = {'prediction_3dmm': prediction_3dmm,
                                    'target_3dmm': target_3dmm,
                                    'prediction_3dmm_overlap_pre': prediction_3dmm_overlap_pre,
                                    'prediction_3dmm_overlap_this': prediction_3dmm_overlap_this}



            # output_whole['target_emotion'] =  l_emotion_selected
            return output_whole
                ...
                for i in range(seq_len):
                    ...
                    with torch.no_grad():
                        output = [output for output in self.decoder_diffusion.ddim_sample_loop_progressive(
                            ...
                            noise=noise,
                            model_kwargs=model_kwargs,
                            shape=(batch_size * self.k, self.window_size, self._3dmm_dim) # past_listener_3dmmenc.shape,  # (batch_size * k, window_size, 58)
            ...
            if not self.is_concat_inference:
                return {"prediction_3dmm": output_listener_3dmm}
            ...
            past_pos = torch.zeros(size=(batch_size, self.k, 1, 6)).to(output_listener_3dmm.device)
            for i in range(seq_len):
                if i == 0:
                    _output_listener_3dmm = torch.cat((_output_listener_3dmm,
                                                       output_listener_3dmm[:, :, i, :, :]), dim=-2)
                    past_pos = output_listener_3dmm[:, :, i, -1, -6:]
                else:
                    this_window = output_listener_3dmm[:, :, i, -self.window_sliding:, :]
                    # print(this_window.shape) # [1, 10, 25, 58]
                    # print(past_pos.shape) # [1, 10, 6]
                    # print(this_window[:, :, 0, -6:].shape) # [1, 10, 6]

                    delay_size = 5
                    # this_window = delay_cat(this_window, delay_size, past_pos)
                    if self.use_interpolation_inference:
                        this_window = delay_cat_cubic_spline(this_window, delay_size, past_pos)
                    # this_window = delay_cat_nonlinear(this_window, delay_size, past_pos)

                    # gap = this_window[:, :, 0, -6:] - past_pos
                    # print(gap.shape) # [1, 10, 6]
                    # this_window[:, :, :, -6:] = this_window[:, :, :, -6:] - gap.unsqueeze(-2)
                    _output_listener_3dmm = torch.cat((_output_listener_3dmm,
                                                       this_window), dim=-2)
                    past_pos = this_window[:, :, -1, -6:]
            ...
            # print(_output_listener_3dmm.shape)

            # # moving_average
            # window_size = 10
            # _output_listener_3dmm[:,:,:,55:] = self.moving_average(_output_listener_3dmm[:,:,:,55:], window_size)
    ...
    def forward(self, **kwargs):
        output_whole = self._forward(**kwargs)
        if self.emotion_decode:
            emotion_prediction = self.latent_3dmm_embedder.coeff_reg(output_whole["prediction_3dmm"])
            output_whole["prediction_emotion"] = emotion_prediction
            # output_whole = {
            #     "prediction_3dmm": output_whole["prediction_3dmm"],
            #     "prediction_emotion": emotion_prediction,
            # }
    ...
    def moving_average(self, x, window_size):
        """
        Apply moving average to the input tensor along the specified dimension.

        Args:
            x (Tensor): Input tensor of shape (batch_size, k, seq_len, dim).
            window_size (int): Size of the moving average window.

        Returns:
            Tensor: Smoothed tensor of the same shape as the input.
        """
        batch_size, k, seq_len, dim = x.shape

        # Create a kernel for the moving average
        kernel = torch.ones(1, 1, window_size) / window_size
        kernel = kernel.to(x.device)

        # Reshape the input tensor to apply the moving average
        x = x.permute(0, 1, 3, 2).contiguous().view(batch_size * k * dim, 1, seq_len)

        padding = (window_size - 1) // 2

        # # Apply the moving average
        # smoothed_x = F.conv1d(x, kernel, padding=padding)

        # Manually pad the input tensor to handle the edges
        if window_size % 2 == 0:
            padding = (window_size - 1) // 2
            x = F.pad(x, (padding, padding + 1), mode='replicate')
        else:
            x = F.pad(x, (padding, padding), mode='replicate')

        # Apply the moving average
        smoothed_x = F.conv1d(x, kernel)

        # Reshape back to the original shape
        smoothed_x = smoothed_x.view(batch_size, k, dim, seq_len).permute(0, 1, 3, 2)
    ...
    def forward(
        ...
        if self.mode in ["train", "val"]:  # train | val
            ...
            output_decoder = self.diffusion_decoder.forward(
                ...
                listener_reference=listener_reference,  # listener's reference pic.
            )
