...
class Render(object):
    ...
    def __init__(self, device='cpu'):
        ...
        self.id_tensor = torch.from_numpy(np.load('external/FaceVerse/reference_full_old.npy')).float().view(1, -1)[:, :150]
        # self.id_tensor = torch.from_numpy(np.load('external/FaceVerse/reference_full_old_data.npy')).float().view(1, -1)[:, :150]
        self.pi_render = FaceGenerator().to(device)
        self.pi_render.eval()
        # checkpoint = torch.load('./external/PIRender/cur_model_fold.pth')
        checkpoint = torch.load('./external/PIRender/p400p_ema.pth')
        ...
        # self.mean_face = torch.FloatTensor(
        #     np.load('external/FaceVerse/mean_face_old_data.npy').astype(np.float32)).view(1, 1, -1).to(device)
        # self.std_face = torch.FloatTensor(
        #     np.load('external/FaceVerse/std_face_old_data.npy').astype(np.float32)).view(1, 1, -1).to(device)
    ...
    def rendering(self, path, ind, listener_vectors, speaker_video_clip, listener_reference):
        ...
        listener_exp_tensor = listener_vectors[:, :52].view(T, -1).to(listener_vectors.get_device()).clone()
        listener_rot_tensor = listener_vectors[:, 52:55].view(T, -1).to(listener_vectors.get_device()).clone()
        listener_trans_tensor = listener_vectors[:, 55:].view(T, -1).to(listener_vectors.get_device()).clone()

        listener_rot_tensor[:, 2] = - listener_rot_tensor[:, 2]
        listener_rot_tensor[:, 1] = - listener_rot_tensor[:, 1]
        # listener_rot_tensor[:, 0] = - listener_rot_tensor[:, 0]
        listener_trans_tensor[:, 0] = - listener_trans_tensor[:, 0]

        self.faceverse.exp_tensor = listener_exp_tensor
        self.faceverse.rot_tensor = listener_rot_tensor
        self.faceverse.trans_tensor = listener_trans_tensor
        self.faceverse.id_tensor = self.id_tensor.view(1, 150).repeat(T, 1).view(T, 150).to(
            listener_vectors.get_device())
    ...
    def rendering_for_fid_depth(self, path, ind, listener_vectors, speaker_video_clip, listener_reference,
                          listener_video_clip, step=30, listener_depth=0):
        # 3D video
        T = listener_vectors.shape[0]
        listener_vectors = self._reverse_transform_3dmm(listener_vectors)[0]

        # listener_vectors [sen_len, 58]
        listener_vectors[:, -1] = listener_vectors[:, -1] - listener_vectors[T//2, -1]
        listener_vectors[:, -1] = listener_vectors[:, -1] + listener_depth
    ...
    def rendering_for_fid_simple(self, path, ind, listener_vectors, speaker_video_clip, listener_reference,
                          listener_video_clip, step=30):
        # 3D video
        T = listener_vectors.shape[0]
        listener_vectors = self._reverse_transform_3dmm(listener_vectors)[0]
        ...
        # 分段处理speaker_video_clip
        def process_speaker_video_in_segments(speaker_video_clip, segment_size=50):
            processed_clips = []
            num_segments = (len(speaker_video_clip) + segment_size - 1) // segment_size  # 计算需要处理的段数

            for i in range(num_segments):
                start_idx = i * segment_size
                end_idx = min((i + 1) * segment_size, len(speaker_video_clip))
                # print(f"Processing segment {i + 1}/{num_segments}, from index {start_idx} to {end_idx}")
                segment = speaker_video_clip[start_idx:end_idx]

                # 对每个segment调用原来的转换函数
                processed_segment = torch_img_to_np2(segment)
                processed_clips.append(processed_segment)

            # 将所有处理过的片段合并
            final_clip = np.concatenate(processed_clips, axis=0)
            return final_clip

        speaker_video_clip = process_speaker_video_in_segments(speaker_video_clip, segment_size=50)
        ...
        # listener_video_clip = process_speaker_video_in_segments(listener_video_clip, segment_size=50)
        ...
        out2 = cv2.VideoWriter(os.path.join(path, 'results_videos', ind + "_val.mp4"), cv2.VideoWriter_fourcc(*"mp4v"),
                              25, (224, 224))
    ...
    def rendering_for_mp4(self, path, ind, listener_vectors, speaker_video_clip, listener_reference,
                                 listener_video_clip, step=30):
        # 3D video
        T = listener_vectors.shape[0]
        listener_vectors = self._reverse_transform_3dmm(listener_vectors)[0]
    ...
    def rendering_for_mp4_all(self, path, ind, listener_vectors, speaker_video_clip, listener_reference,
                                 listener_video_clip, step=30):
        # 3D video
        T = listener_vectors.shape[0]
        listener_vectors = self._reverse_transform_3dmm(listener_vectors)[0]
