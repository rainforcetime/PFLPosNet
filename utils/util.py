...
def get_epoch_num_from_path(path):
    # exp_1000/epoch_{1000}_checkpoint.pth
...
def nanmean(x, dim=None):
    # 将 NaN 值替换为零
    x_no_nan = torch.where(torch.isnan(x), torch.zeros_like(x), x)

    # 计算非 NaN 值的数量
    if dim is not None:
        non_nan_count = torch.sum(~torch.isnan(x), dim=dim)
        non_nan_count = non_nan_count.unsqueeze(dim)
    else:
        non_nan_count = torch.sum(~torch.isnan(x))

    # 计算 nanmean
    if dim is not None:
        return torch.sum(x_no_nan, dim=dim) / non_nan_count
    else:
        return torch.sum(x_no_nan) / non_nan_count
...
def plot_supcon_similarity(feat, label, writer, iter, split, save_dir):
    ...
    # 将结果写入 TensorBoard
    writer.add_scalar("{}/Similarity/positives".format(split), nanmean(positives), iter)
    writer.add_scalar("{}/Similarity/negatives".format(split), nanmean(negatives), iter)
...
def save_checkpoint(cfg, model, optimizer, epoch, is_best, is_current=False):
...
def checkpoint_resume(cfg, model, device):
    ...
    # 检查是否有缺少的键值
    resume_state_dict = checkpoint['state_dict']
    model_state_dict = model.state_dict()
    missing_keys = [k for k in model_state_dict.keys() if k not in resume_state_dict]
    if missing_keys != []:
