...
class SupConLoss(nn.Module):
    ...
    def forward(self, feature, label, device):
        ...
        mask_pos_pairs = torch.where(mask_pos_pairs < 1e-6, torch.tensor(1.0, dtype=torch.float32).to(device), mask_pos_pairs)
...
def DepthLoss(prediction, target):
    # print(prediction.shape, target.shape)

    assert len(prediction.shape) == len(target.shape), "prediction and target must have the same shape"
    assert len(prediction.shape) == 2, "Only works with predictions of shape [batch_size, seq_length, features]"
