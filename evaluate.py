...
def parse_arg():
    ...
    parser.add_argument("--epoch_num", type=int, help="epoch number of saving model weight", default=1000)
    parser.add_argument("--exp_num", type=int, help="the number of training experiment.", default=1000)
    parser.add_argument("--mode", type=str, help="train (val) or test", default="test")
    parser.add_argument("--config", type=str, help="config path", default="./config/val_diffusion.yaml")
...
def evaluate(cfg, device, model, test_loader, render, split):
    ...
    # _3dmm_pred = all_listener_3dmm_pred.detach().cpu().numpy()
    # min_value = np.min(_3dmm_pred)
    # max_value = np.max(_3dmm_pred)
    # mean_value = np.mean(_3dmm_pred)
    # print("Max_value: {:.5f}  Min_value: {:.5f} Mean_value: {:.5f}".
    #       format(max_value, min_value, mean_value))
    # logging.info("Max_value: {:.5f}  Min_value: {:.5f} Mean_value: {:.5f}".
    #              format(max_value, min_value, mean_value))
    #
    # _3dmm_gt = all_listener_3dmm_gt.detach().cpu().numpy()
    # min_value = np.min(_3dmm_gt)
    # max_value = np.max(_3dmm_gt)
    # mean_value = np.mean(_3dmm_gt)
    # print("Max_value: {:.5f}  Min_value: {:.5f} Mean_value: {:.5f}".
    #       format(max_value, min_value, mean_value))
    # logging.info("Max_value: {:.5f}  Min_value: {:.5f} Mean_value: {:.5f}".
    #              format(max_value, min_value, mean_value))

    # TODO: Save the prediction and ground truth
    # np.save(os.path.join(out_dir, "all_speaker_3dmm.npy"),
    #         all_speaker_3dmm.cpu().numpy())
    # np.save(os.path.join(out_dir, "all_listener_3dmm_pred.npy"),
    #         all_listener_3dmm_pred.cpu().numpy())
    # np.save(os.path.join(out_dir, "all_listener_3dmm_gt.npy"),
    #         all_listener_3dmm_gt.cpu().numpy())
