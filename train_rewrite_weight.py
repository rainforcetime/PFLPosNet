...
import numpy as np
os.environ['CUDA_VISIBLE_DEVICES'] = '1'  # Make only the first GPU visible
...
def parse_arg():
    ...
    parser.add_argument("--config", type=str, help="config path", default="config/train_rewrite_weight.yaml")
...
def main(args):
    ...
    for epoch in range(cfg.trainer.start_epoch, cfg.trainer.epochs):
        ...
        # TODO: debug: save parameters of hypernet per epoch
        # import json
        # for k, v in main_model.hypernet.named_parameters():
        #     if k == 'branches.0.bias':
        #         weight = v[:1000].detach().cpu().tolist()
        #         param_dict = {'param': weight}
        #         file_dir = "./results/temp_save/exp_{}".format(cfg.exp_num)
        #         os.makedirs(file_dir, exist_ok=True)
        #         file_path = os.path.join(file_dir, "epoch_{}_param.json".format(epoch + 1))
